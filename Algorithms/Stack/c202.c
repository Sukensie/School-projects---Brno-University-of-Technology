...
void Stack_Init( Stack *stack ) {
	...
	stack->topIndex = -1; //-1 značí prázdný stack
}
...
int Stack_IsFull( const Stack *stack ) {
	...
	return	stack->topIndex == (STACK_SIZE-1) ? 1 : 0; //(stack_size-1) protože index posleního prvku je vždy o 1 menší než velikost
...
void Stack_Top( const Stack *stack, char *dataPtr ) {
    ...
    *dataPtr = stack->array[stack->topIndex]; //zajímá mě prvek na vrcholu (na topIndexu)
}
...
void Stack_Pop( Stack *stack ) {
	...
	//pokud je stack prázdný, není co popnout
	if(!Stack_IsEmpty(stack))
	{
		stack->topIndex--;
	}
...
void Stack_Push( Stack *stack, char data ) {
	...
	//pushnutí do plného stacku by způsobilo chybu
