...
#all necessary matrix or vector operations can be found at https://docs.blender.org/api/current/mathutils.html
    ...
    def createView():
        ...
        upVector =  mathutils.Vector((0.0, 1.0, 0.0))      
        ...
        rot = mathutils.Matrix.Rotation(0.226892803, 4, 'Z')
        Z = rot @ Z   
        Z.normalize()
    ...
    def createTransformation(object, view):
        ...
        #######################
        rotateMatrix = mathutils.Matrix.Rotation(1.57079633, 4, 'X') #bug fix, jinak by to zůstalo špatně orotované
        translationMatrix = mathutils.Matrix.Translation((objectNumber-4, 0.0, 0.0))
        if objectNumber > 3:
            translationMatrix = mathutils.Matrix.Translation((objectNumber-4, 1.0, 0.0))
        transformation = rotateMatrix @ view @ translationMatrix
        #######################
